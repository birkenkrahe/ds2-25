...
* Week 9: ~break~ ~next~ ~repeat~ and ~function~ deep dive

- [ ] Any feedback on the Literature Review sprint review?

- [ ] *Tests*: Former rule (50% after the deadline) reinstalled (because
...
- [ ] *Test 5* is live: 50 questions on loops, projects, utilities. Uses
  lectures and review sessions (available in GitHub) as
  usual. Deadline March 21 (2 weeks).

- [ ] Loop control with ~break~, ~next~ and ~repeat~. Get:
