...
** DONE Review & Practice: Arguments / Loops
...
** DONE DataCamp Review - Functions
...
** DONE Home assignment (by Monday 17 Feb) - Canvas
...
** DONE In-class assignment conditional statements (Canvas upload)
...
2. What does this code produce?
   #+begin_src R :session *R* :results output :exports both
     mat <- diag(2)
     ifelse(mat > 0, "High", "Low")
   #+end_src

   #+begin_quote
   - The shape of =mat= is a 2 x 2 matrix. =ifelse= retutns such a matrix.
