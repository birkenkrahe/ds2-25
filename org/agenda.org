...
* NEXT Week 11: Writing functions like a pro
#+attr_html: :width 400px :float nil:
...
- [ ] DataCamp course "Writing functions in R" extended to Friday
- [ ] 2nd sprint review
- [ ] DataCamp review: functions (only chapter 1)
- [ ] "Tidyverse" vs. Base R
- [ ] No class meeting on Tuesday, April 22 (Honors Convocation)

** TODO 2nd Sprint Review - the key lessons as cartoons
...
** TODO DataCamp review - How to Write a Function
...
3) Demonstrate the ~trim~ parameter of the ~mean~ function with a suitable
   ...
   #+begin_src R :session *R* :results output :exports both
     mean(foo)
     mean(foo,trim=0.25) # drop 25% or 1 value from either side
     sort(foo)[c(-1,-5)]
     mean(sort(foo)[c(-1,-5)])
     mean(foo,trim=0.5) # drop 50% or int(5/2)=2 values from either side
     sort(foo)[c(-1,-2,-4,-5)]
...
5) Ellipsis arguments for ~mean~ - apart from ~na.rm~ - what're they for?
   ...
   #+end_quote
   #+begin_src R :session *R* :results output :exports both
     ## create a `verbose`/chatty `mean` function
     mean.myclass <- function(x, ..., verbose=FALSE) {
       if (verbose) {
         cat("Calculating mean for object of class 'myclass'\n")
       }
       mean(as.numeric(x)) # default mean computation
     }

     mean(1:10)  # calls mean.default()
     mean(1:10,verbose=TRUE)  # calls mean.default()

     x <- structure(1:10,class="myclass")
     mean(x,verbose=TRUE) # cals mean.myclass()
   #+end_src
...
** TODO "Tidyverse" vs Base R Comparison (Grok)

Based on the example of importing and processing test scores from a
CSV file (to be found in DataCamp and here).
...
- [ ] Writing functions for new functionality
- [ ] Long and short version of functions
...
* Week 12: Data science on the command-line
* Week 13: Improving performance and debugging
* Week 14: Optimizing R Code with Rcpp
* Week 15: Interfacing R with SQL Databases
* Week 16: Interfacing R with Python
