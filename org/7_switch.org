...
- *Good practice:* When the program logic gets more complex (> lines or
  > 2 nested functions or conditions), resort to pseudocode (prose or
...
- Nesting different statements for two variables ~a~ and ~mynumber~ (Note
  ...
  are unknown.)
  #+name: nested
  #+begin_src R :results silent :tangle ../src/nested.R
    if(a <= mynumber) {
...
1) Tangle the code block named [[nested]] above as a script =nested.R= and
   try to run it on the command-line with =Rscript=.
